...
def plot_stacked_bar_across_model_and_data(experiment, pd_word_probs_over_situations, language, model, origin):
    ...
    if experiment == "attention":
        pd_selection.set_index('listener_att').plot(kind='bar', stacked=True, color=my_colors)
    else:
        pd_selection.set_index('listener_pos').plot(kind='bar', stacked=True, color=my_colors)
    ...
    if transparent_plots is True:
        ...
        # add axis titles
        if experiment == "attention":
            plt.xlabel('Listener attention', fontsize=16)
        else:
            plt.xlabel('Listener position', fontsize=16)
        if origin == "model predictions":
    ...
    plt.xticks([0, 1, 2, 3], [1, 2, 3, 4], rotation=0, fontsize=15)
