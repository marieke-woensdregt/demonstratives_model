...
# PARAMETER SETTINGS: #
languages = ["English", "Portuguese"]  # ["English", "Italian", "Portuguese", "Spanish"]
...
def predictions_over_situations(measure, pd_model_predictions_baseline, pd_model_predictions_attention, model, language, WordNo, speaker_tau, listener_tau, object_positions, listener_attentions):
    ...
    for object_pos in object_positions:
        for listener_att in listener_attentions:
            ...
            if measure == "probability":
                probs_per_word_baseline, probs_per_word_attention = get_model_predictions(pd_model_predictions_baseline, pd_model_predictions_attention, model, language, speaker_tau, listener_tau, object_pos, listener_att)
...
def prediction_difference_across_parameter_settings(measure, model, language, WordNo, object_positions, listener_attentions, tau_start, tau_stop, tau_step):
    if WordNo == 2:
    ...
    for listener_rationality in np.arange(tau_start, tau_stop, tau_step):
        ...
        for speaker_rationality in np.arange(tau_start, tau_stop, tau_step):
            ...
            pd_difference_per_situation = predictions_over_situations(measure, model_predictions_baseline, model_predictions_attention, model, language, WordNo, round(speaker_rationality, 2), round(listener_rationality, 2), object_positions, listener_attentions)
...
best_fit_parameters_exp2_dict = {"English":[1.7, 1.15],
                                 ...
                                 "Spanish":[0.5, 1.95]}
for parameter_dict in [best_fit_parameters_exp1_dict, best_fit_parameters_exp2_dict]:
    print('')
    print('')
    print('')
    ...
    for language in languages:
        ...
        pd_difference_per_situation = predictions_over_situations(measure, model_predictions_baseline, model_predictions_attention, model, language, WordNo, speaker_tau, listener_tau, object_positions, listener_attentions)
...
for language in languages:
    ...
    pd_difference_across_parameters = prediction_difference_across_parameter_settings(measure, model, language, WordNo, object_positions, listener_attentions, tau_start, tau_stop, tau_step)
    pd.set_option('display.max_columns', None)
    print('')
    print('')
