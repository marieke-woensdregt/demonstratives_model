...
for listener_rationality in np.arange(tau_start, tau_stop, tau_step):
	...
	for speaker_rationality in np.arange(tau_start, tau_stop, tau_step):
		...
		LS = LiteralSpeaker_MW.LiteralSpeaker(n_objects=len(listener_attentions), stau=speaker_rationality,ltau=listener_rationality,verbose=False) #TODO: Move the rounding to here instead of elsewhere?
		for model in models:
...
output_file_name = 'HigherSearchD_MW_RSA_Attention_n_positions_'+str(len(listener_attentions)) + str(models).replace(" ", "") + '_tau_start_' + str(tau_start) + '_tau_stop_' + str(tau_stop) + '_tau_step_' + str(tau_step) + '.csv'
